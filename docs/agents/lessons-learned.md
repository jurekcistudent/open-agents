...
- Tool part inputs from external harnesses (codex/claude-code bridge) are untyped JSON — the AI SDK schema never validated them, so `part.input as AskUserQuestionInput` casts are unsafe and models can send stringified or non-array nested fields. Normalize at the render boundary (`lib/chat/normalize-ask-user-question.ts`) instead of calling `.map` on trusted casts, or the whole chat page crashes.
